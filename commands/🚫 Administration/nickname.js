    ...
    try {
      if(!message.guild.me.permissions.has([Permissions.FLAGS.MANAGE_NICKNAMES]))      
        return message.reply({embeds : [new MessageEmbed()
          ...
          .setTitle(`<a:cross1:953179133511335976> **I am missing the Permission to Manage Nicknames of others**`)
        ]})
      ...
      if (!kickmember)
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<a:cross1:953179133511335976> **You forgot to ping a Member**`)
      ...
      if(!args[1])
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<a:cross1:953179133511335976> **You forgot to add a Nickname**`)
          .setDescription(`Usage: \`${prefix}nickname @User newnickname\``)
        ]});
      ...
      if(nickname.length > 32)
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<a:cross1:953179133511335976> **The Nickname must have smaller then 32 Characters**`)
          .setDescription(`Usage: \`${prefix}nickname @User newnickname\``)
        ]});
      kickmember.setNickname(nickname).then(member => {
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.color)
          .setFooter(client.getFooter(es))
          .setTitle(`<a:tick:955870506337329182> **Successfully changed the username of \`${kickmember.user.tag}\` to \`${nickname}\`**`)
        ]});
      }).catch(e=>{
